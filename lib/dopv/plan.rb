  ...
  class Plan
    def self.load(plan)
      ...
      when Hash
        ...
        raise Dopv::Errors::PlanError, "Plan argument must be of String or Hash type"
      end
    end
    ...
    def validate
      ...
      # definitions.
      raise Dopv::Errors::PlanError, 'Plan must be of hash type' unless @plan.is_a?(Hash)
      if !@plan.has_key?('clouds') || !@plan.has_key?('nodes')
        raise Dopv::Errors::PlanError, "Plan must define 'clouds' and 'nodes' hashes"
      end
      # Cloud and node definitions must be groupped into hashes.
      if !@plan['clouds'].is_a?(Hash) || !@plan['nodes'].is_a?(Hash)
        raise Dopv::Errors::PlanError, 'Nodes and clouds must be of hash type'
      end
      @plan['clouds'].each do |n, c|
        raise Dopv::Errors::PlanError, "Cloud #{n} is of unsupported type" unless Dopv::Cloud.supported?(c)
      end
      ...
      @plan['nodes'].each do |n, d|
        if !(d.is_a?(Hash) && d['cloud'].is_a?(String) && d['flavor'].is_a?(String) &&
          ...
          raise Dopv::Errors::PlanError, "Invalid definition of node #{n}"
        end
        raise Dopv::Errors::PlanError, "Invalid cloud definition for node #{n}" unless @plan['clouds'].has_key?(d['cloud'])

        # Networks
        d['nets'].each do |net|
          if !net.is_a?(Hash) || !net['int'].is_a?(String) || !net['proto'].is_a?(String)
            raise Dopv::Errors::PlanError, "Invalid network definition for node #{n}"
          end
          if net['proto'] != 'static' && net['proto'] != 'dhcp'
            raise Dopv::Errors::PlanError, "Invalid network protocol definition for node #{n}"
          end
          if net['proto'] == 'static'
            #binding.pry
            begin
              ...
              raise Dopv::Errors::PlanError, "Either of ip, netmask, gateway is invalid or indefined for node #{n}"
            end
          end
        end
        ...
        # Disks
        if d.has_key?('disks')
          raise Dopv::Errors::PlanError, "Invalid disk definition of node #{n}" unless d['disks'].is_a?(Hash)
          d['disks'].each do |dsk,size|
            if !dsk.is_a?(String) || size !~ /[1-9]*[MGTmgt]/
              raise Dopv::Errors::PlanError, "Invalid disk name and/or size definition of node #{n}"
            end
          end
        end
        ...
        if d.has_key?('dns')
          if !d['dns'].is_a?(Hash) || !d['dns']['nameserver'].is_a?(Array)
            raise Dopv::Errors::PlanError, "Invalid dns specification of node #{n}"
          end
          d['dns']['nameserver'].each do |srv|
            begin
              IPAddr.new(srv)
            rescue
              raise Dopv::Errors::PlanError, "Invalid name server definition of node #{n}"
            end
          end
          if d['dns'].has_key?('domain') && !d['dns']['domain']
            raise Dopv::Errors::PlanError, "Invalid search domain definition of node #{n}"
          end
        end
      end
    end
